...
void Stage::initialize(std::string filename)
{
    ...
    while(stage_file)
    {
        ...
        char *lin = new char[line.size()+1];        //converting string to a char * to pass to strtok(line.c_str() gives a const char *)
        ...
        if(parameters[0]<=10)
            temp.gol=new Goli(parameters[0],SCREEN_HEIGHT - parameters[1],9,"ball.png",parameters[2],parameters[3],parameters[4]);
        else
            temp.gol=new Goli(SCREEN_WIDTH - parameters[0],SCREEN_HEIGHT - parameters[1],9,"ball.png",-parameters[2],-parameters[3],parameters[4]);
    ...
    for(i=0;i<no_of_bullets;i++)
    {
        if(bullet[i].exist==false)
        {
            ...
            if(rand()%2)
               ...
               bullet[i].gol= new Goli(10,SCREEN_HEIGHT - height[rand()%7],9,"ball.png",speed[rand()%5],0,0);
